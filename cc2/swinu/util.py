...
def flow_roll_forecast(
    ...
    for t in range(n_step):
        ...
        alphas = torch.linspace(alpha_init, 0.05, num_inference_steps + 1, device=x.device)
    ...
    out = {
        "tendencies_core": torch.cat(all_tendencies, dim=1),
        "predictions_core": torch.cat(all_predictions, dim=1),
        "tendencies_obs": torch.cat(all_tendencies, dim=1),
        "predictions_obs": torch.cat(all_predictions, dim=1),
    }
