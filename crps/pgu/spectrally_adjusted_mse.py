...
def amse2d_with_tendency_loss_somple(
    y_true: torch.Tensor,
    y_pred: torch.Tensor,
    *,
    alpha: float = 2.0,  # tendency weight (optional)
    ...
    # ---- spectral AMSE (paper-style) ----
    amse_t = _amse2d_per_time(  # uses the bf16-safe version from (A)
        y_pred, y_true, n_bins=n_bins, hann_window=hann_window, eps=eps
    )  # [T]

    # ---- tendency term built on MSE base (no pixelwise step MSE added) ----
    # per-pixel MSE
    base_mse = (y_pred - y_true).pow(2)  # [B,T,C,H,W]

    # your magnitude weighting
    magnitude = (torch.abs(y_true) - tau).clamp_min(0.0)  # [B,T,C,H,W]
    magnitude_mean = magnitude.mean(dim=(0, 2, 3, 4), keepdim=True)  # [1,T,1,1,1]
    magnitude = magnitude / (magnitude_mean + 1e-8)
    magnitude = magnitude * alpha

    tendency_loss = (magnitude * base_mse).mean(dim=(0, 2, 3, 4))  # [T]

    # ---- final loss = AMSE + tendency ----
    loss = amse_t.mean() + tendency_loss.mean()    
    assert torch.isfinite(loss), f"Non-finite values at loss: {loss}"
