...
from layers import (
    ...
    PatchEmbeddingWithConv,
    PatchRecoveryWithConv,
)
...
class CloudCastV2(nn.Module):
    def __init__(self, patch_size, dim, stride=None, num_mix=2):
        ...
        self.patch_embed = PatchEmbeddingWithConv(dim)
        ...
        self.patch_recover = PatchRecoveryWithConv(
            dim=dim,
            output_dim=3 * num_mix,
        )
        # self.patch_recover = PatchRecovery(
        #    dim=dim,
        #    output_dim=3*num_mix, #dim,
        #    patch_size=patch_size,
        # )

        self.prediction_head = MixtureBetaPredictionHead(num_mix)
